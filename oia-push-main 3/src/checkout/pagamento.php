...
try {
    ...
    error_log("[Nivopay] 🌐 URL da requisição: " . $apiUrl . '/transaction.purchase');
    error_log("[Nivopay] 📦 Dados enviados: " . json_encode($data));
    ...
    error_log("[Nivopay] 🔍 Detalhes da requisição cURL:\n" . $verboseLog);

    if ($curlError) {
        error_log("[Nivopay] ❌ Erro cURL: " . $curlError . " (errno: " . $curlErrno . ")");
        throw new Exception("Erro na requisição: " . $curlError);
    }

    curl_close($ch);

    error_log("[Nivopay] 📊 HTTP Status Code: " . $httpCode);
    error_log("[Nivopay] 📄 Resposta bruta: " . $response);
    ...
    error_log("[Nivopay] 💳 Transação criada com sucesso: " . $result['id']);
    error_log("[Nivopay] 📄 Resposta completa da API: " . $response);
    error_log("[Nivopay] 🔑 Token gerado: " . $result['id']);
    ...
    error_log("[Nivopay] 📤 Enviando resposta ao frontend: " . json_encode($responseData));
    echo json_encode($responseData);

} catch (Exception $e) {
    error_log("[Nivopay] ❌ Erro: " . $e->getMessage());
    error_log("[Nivopay] 🔍 Stack trace: " . $e->getTraceAsString());
